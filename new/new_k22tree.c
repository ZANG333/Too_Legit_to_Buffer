 ...
 *
 * Return:
 * * ret_val - Number of entries written in the kbfuf buffer 
...
static int dfs(struct k22info *kbuf, int max)
{
	...
	while (!list_empty(&stack)) {
		...
		strscpy(kbuf[count].comm, curr->task->comm, sizeof(kbuf[count].comm));
...
static int do_k22tree(struct k22info *buf, int *ne)
{
	...
	ksize = min(number_processes + 5, size);

	kbuf = kcalloc(ksize, sizeof(struct k22info), GFP_KERNEL);
	if (!kbuf) {
		ret_val = -ENOMEM;
		goto out;
	}

	ret_val = dfs(kbuf, ksize);
	if (ret_val < 1) {
		ret_val = -EINVAL;
		goto out;
	}
	...
	if (copy_to_user(buf, kbuf, ksize * sizeof(struct k22info))) {
		ret_val = -EFAULT;
		goto out;
	}

	if (ksize < size){
		if (copy_to_user(ne, &ksize, sizeof(int))) {
			ret_val = -EFAULT;
			goto out;
		}
	}
